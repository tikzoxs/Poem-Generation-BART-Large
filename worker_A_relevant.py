...
import json
import redis
from speechgenerator import text_to_voice
...
good_words = ['them', 'hang', 'need', 'new', 'above', 'hang', 'floor', 'wait', 'audience', 'out', 'becauase', 'seem',
              'meet', 'heart',
              'curiuos', 'play', 'people', 'tell', 'hear', 'surprise', 'element', 'keep', 'moemory', 'memories',
              'thing', 'different',
              'courage', 'there', 'tales', 'many', 'happen', 'trust', 'unknown', 'situation', 'day', 'put',
              'day', 'gone', 'darkness', 'darkness', 'fall', 'children', 'hard', 'blossom', 'world', 'protect',
              'outside', 'harsh',
              'away', 'any', 'garden', 'secret', 'retaliation', 'home', 'attack', 'trigger', 'provication', 'beware',
              'your', 'grow', 'flowers', 'you',
              'watch', 'comfort', 'calm', 'embrace', 'words', 'offer', 'wonderful', 'around', 'feels', 'warm', 'roses',
              'shelter', 'silk', 'take', 'mighty', 'flow',
              'us', 'butterflies', 'soft', 'let', 'heaven', 'shining', 'summer', 'wonder', 'expression', 'flutter',
              'their', 'glittering', 'like', 'full',
              'weightless', 'morning', 'hope', 'jump', 'bubbles', 'energy', 'run', 'tale', 'explore', 'spark', 'dance',
              'fly', 'wings', 'filed', 'sparks', 'fileds',
...
emotion_list = [['peace', 'neutral', 'calm', 'relax'], ['happy', 'joy', 'happiness', 'over the moon'],
                ['anger', 'angry', 'hate', 'lonely', 'sad', 'fear', 'negativity', 'unkind', 'brutal'],
                ['care', 'love', 'affection', 'kind', 'compassion']]
embeddings_dict = {}

sentiments_list = ['POSITIVE', 'NEGATIVE', 'NEUTRAL']
numbers_list = ['SINULAR', 'PLURAL', 'BACKUP', 'GENERAL']

prompt_list = ["Did you say something? I didn't hear", "I don't like this silence, is someone there?",
               "I'm in darkness, am I deaf?, Hey, you."]
instruction_list = ["Please speak up.", "tell me something.", "talk louder", "speak your heart out", "tell a story",
                    "speak louder"]
...
def syntax_analysis_local(good_candidate_words_for_question, okay_noun_candidate_words):
    if(len(good_candidate_words_for_question) > 0):
        return good_candidate_words_for_question
    elif(len(okay_noun_candidate_words) > 0):
        return okay_noun_candidate_words
    # elif(len(okay_verb_candidate_words_for_question) > 0): #VERB questions are not yet implemented
    #   return okay_verb_candidate_words_for_question
    else:
        return []

def word_analyzer(in_text):
    good_candidate_words_for_question = []
    okay_noun_candidate_words = []
    okay_verb_candidate_words = []
    entity_words = list(analyze_entities(in_text))
    words, word_types, numbers = analyze_syntax(in_text)
    for word, word_type, number in zip(words, word_types, numbers):
        if(word in entity_words):
            ...
            if(word_type == 'NOUN'):
                okay_noun_candidate_words.append((word,number))
            elif(word_type == 'VERB'):
                okay_verb_candidate_words.append((word,number))
    return good_candidate_words_for_question, entity_words, okay_noun_candidate_words, okay_verb_candidate_words
...
def process_input(in_text, entity_words, okay_noun_candidate_words, okay_verb_candidate_words):
    # input_words = []
    entities_and_nouns_and_verbs = []
    total_length = len(in_text)
    # input_words = entity_words.copy()
    entities_and_nouns_and_verbs_with_number = entity_words + okay_noun_candidate_words + okay_verb_candidate_words
    for env_item in entities_and_nouns_and_verbs_with_number:
        if(len(env_item[0])>1):
            entities_and_nouns_and_verbs.append(env_item[0])
    #print(entity_words)
    ...
    if(entity_count > max_user_input_size):
        ...
        random.shuffle(entities_and_nouns_and_verbs)
        if(len(entities_and_nouns_and_verbs) > max_user_input_size):
            final_words = entities_and_nouns_and_verbs[0:max_user_input_size]
        else:
            final_words = entities_and_nouns_and_verbs
    ...
    #print("processed in text : ", processed_in_text)
...
def care_or_happy(out_text):
    happiness_metric = len(re.findall('happiness', out_text)) + len(re.findall('happy', out_text)) + len(
        re.findall('joy', out_text)) + len(re.findall('fun ', out_text)) + len(re.findall('funny ', out_text)) + len(
        re.findall('fun.', out_text)) + len(re.findall('fun,', out_text))
    love_metric = len(re.findall('love', out_text)) + len(re.findall('care', out_text)) + len(
        re.findall('caring', out_text)) + len(re.findall('embrac', out_text)) + len(re.findall('affection', out_text))
...
if __name__ == "__main__":
    ...
    while (True):
        #print("\n\n*****************************************************\n\n")
        tmp = redis_client.brpop("job")
        print("A task incoming.")
        task = json.loads(tmp[1])
        job_id = task['id']
        in_text = task['input']

        good_candidate_words_for_question, entity_words, okay_noun_candidate_words, okay_verb_candidate_words = word_analyzer(in_text)
        ARTICLE_TO_SUMMARIZE = process_input(in_text,entity_words, okay_noun_candidate_words, okay_verb_candidate_words)
        ...
        whole_out_text = [tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=False) for g in
                          summary_ids]
        ...
        candidates = syntax_analysis_local(good_candidate_words_for_question, okay_noun_candidate_words)
        question = generate_question(candidates, pavilion_emotion)

        vid = job_id + "_v"

        voice_bytes = text_to_voice(final_output, question, pavilion_emotion)
        redis_client.set(vid, voice_bytes, ex=120)

        response = {'question': question, 'emotion': pavilion_emotion, 'output': final_output, 'vid': vid}
        redis_client.set(job_id, json.dumps(response), ex=120)
